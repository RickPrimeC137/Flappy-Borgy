...
// --- CORS ---
// Autorise: localhost, 127.0.0.1, IP locales (LAN), et tes domaines Render.
// (Utile quand tu testes depuis http://172.xx.xx.xx:3000 par ex.)
...
function isPrivateIp(hostname) {
  ...
  const m = hostname.match(/^172\.(\d{1,2})\.(\d{1,3})\.(\d{1,3})$/);
  if (m) {
    const b = Number(m[1]);
...
function isAllowedOrigin(origin) {
  if (!origin) return true; // curl/postman / certains webviews
  try {
...
function normMode(m) {
  if (typeof m !== "string") return "normal";
  const s = m.trim().toLowerCase();
  if (s === "hard") return "hard";
  if (s === "normal") return "normal";

  // Daily Run: "daily:YYYY-MM-DD"
  if (s.startsWith("daily:")) {
    const day = s.slice("daily:".length);
    if (/^\d{4}-\d{2}-\d{2}$/.test(day)) return `daily:${day}`;
  }
  return "normal";
}
...
app.post("/api/score", async (req, res) => {
  try {
    ...
    // Calcule rank & total pour ce mode (utile pour bonus classement côté client)
    let rank = null;
    let total = null;
    try {
      const { count: betterCount, error: betterErr } = await supabase
        .from("scores")
      ...
      if (!betterErr && typeof betterCount === "number") rank = betterCount + 1;
      if (!totalErr && typeof totalCount === "number") total = totalCount;
    } catch (e) {
      console.warn("[DB] rank/total compute warn", e);
    }

    return res.json({ ok: true, best: bestOut, rank, total });
  } catch (e) {
...
app.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    let period = parsePeriod(req.query);
    if (mode.startsWith("daily:")) period = "global";
