...
// Mode développement local : permet de démarrer sans credentials pour tester le jeu
const DEV_MODE = !BOT_TOKEN || !SUPABASE_URL || !SUPABASE_SERVICE_ROLE;

if (DEV_MODE) {
  console.warn("⚠️  Mode développement local activé (credentials manquants)");
  console.warn("⚠️  Les routes API (/api/score, /api/leaderboard, /api/me) sont désactivées");
  console.warn("⚠️  Le jeu fonctionne mais sans fonctionnalités en ligne");
}
...
// Middleware pour bloquer les routes API en mode dev
const requireProdMode = (req, res, next) => {
  if (DEV_MODE) {
    return res.status(503).json({
      ok: false,
      error: "API désactivée en mode développement local",
      devMode: true
    });
  }
  next();
...
// Health & root
app.get("/", (_req, res) =>
  res.json({ ok: true, service: "flappyborgy-leaderboard", devMode: DEV_MODE })
);
app.get("/api/health", (_req, res) => res.json({ ok: true, devMode: DEV_MODE }));

// POST /api/score  { score:number, initData:string, mode?: "hard"|"normal" }
app.post("/api/score", requireProdMode, async (req, res) => {
  try {
 ...
 */
app.get("/api/leaderboard", requireProdMode, async (req, res) => {
  try {
...
app.get("/api/me", requireProdMode, async (req, res) => {
  try {
