...
class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.physics.add.overlap(this.player, this.pipes, (_p, pipe) => this.gameOver(pipe), null, this);
  ...
  gameOver(hitPipe){
    ...
    // garde la paire de tuyaux touchée pour l'animation de game over
    if (hitPipe && !this.hitPipePair && this.pipePairs?.length) {
      this.hitPipePair = this.pipePairs.find(pp => pp?.top === hitPipe || pp?.bottom === hitPipe) || null;
    }
  ...
  _finalGameOver(){
    ...
    const hit = this.hitPipePair;
    if (hit && (hit.top || hit.bottom)) {
      // On détruit tout sauf la paire touchée
      this.pipes.children.iterate(p => { if (p && p !== hit.top && p !== hit.bottom) p.destroy(); });
      this.pipePairs = [hit];

      // Stop la physique sur les tuyaux gardés (on va les tweener)
      [hit.top, hit.bottom].forEach(p => {
        if (!p) return;
        try {
          if (p.body) {
      ...
      // Effet "tuyaux qui descendent lentement" pendant le game over
      const Hh = this.scale.height;
      const tTop = hit.top;
      const tBot = hit.bottom;
      if (tTop) this.tweens.add({
        targets: tTop,
        y: Hh + (tTop.displayHeight || tTop.height || 0),
        duration: 1400,
        ease: "Cubic.out"
      });
      if (tBot) this.tweens.add({
        targets: tBot,
        y: Hh + (tBot.displayHeight || tBot.height || 0),
        duration: 1400,
        ease: "Cubic.out"
      });
    } else {
