...
// --- Titre ---
// (1 seule espace entre les mots + un peu plus haut comme sur la référence)
this.add.text(cx, H * 0.095, "FLAPPY BORGY", {
...
this.add.text(cx, H * 0.145, `${t("HUD_COINS")} ${totalCoins}`, {
  fontFamily:"monospace",
  fontSize: 24,
  color:"#ffffff",
  stroke:"#000000",
...
// un peu moins larges/hauts comme sur la capture de référence
const bigW = Math.round(W * 0.56);
const bigH = 80;

// Play
this.makeBtn(cx, H * 0.40, t("MENU_PLAY"), () => this.scene.start("game"), {
  width: bigW,
  height: bigH,
  fontSize: 38,
  bg: "#12a38a",
...
// on supprime le ":" pour coller à l'exemple ("Mode Hard ON/OFF")
const hardLabel = () => (
  this.game._hardMode
    ? `🔥  ${t("MENU_HARD_ON").replace(" :", "")}`
    : `🔥  ${t("MENU_HARD_OFF").replace(" :", "")}`
);
const hardBtn = this.makeBtn(cx, H * 0.49, hardLabel(), () => {
...
this.makeBtn(cx, H * 0.58, `🛒  ${t("MENU_SHOP")}`, () => this.showShop(), {
  width: bigW,
  height: bigH,
  fontSize: 30,
  bg: "#6d28d9",
...
this.makeBtn(cx, H * 0.68, `📜  ${t("MENU_QUESTS")}`, () => this.showQuests(), {
  width: bigW,
  height: bigH,
  fontSize: 30,
  bg: "#334155",
...
this.makeBtn(cx, H * 0.78, `🏆  ${t("MENU_LEADERBOARD")}`, () => {
...
const langSmall = this.makeBtn(leftX, baseY + smallGap * 2, (currentLang() === "fr" ? "🌐  Français" : "🌐  English"), () => {
...
// boutons courts ("Vote" / "Buy") comme sur ta référence, pour éviter le texte qui dépasse
this.makeBtn(rightX, baseY + smallGap, `⭐  Vote`, () => {
...
this.makeBtn(rightX, baseY + smallGap * 2, `💎  Buy`, () => {
