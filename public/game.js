...
class PreloadScene extends Phaser.Scene {
  ...
  preload(){
    ...
    // Cochon (apparition uniquement en mode GOLD pendant la partie)
    // 👉 Ajoute le fichier dans /assets/gold_pig.png
    this.load.image("gold_pig", "gold_pig.png");
...
class MenuScene extends Phaser.Scene {
  ...
  create(){
    ...
    // ⚠️ Fix clics "seulement sur le côté gauche" :
    // si un GameObject (invisible / overlay) capte l'input au-dessus des boutons,
    // on autorise la propagation aux autres objets interactifs.
...
makeMenuBtn(x, y, label, cb, opts = {}) {
  const {
    ...
    minWidth = null,
    minHeight = null,
    fitText = true,            // si true: la barre "colle" au texte (comme sur ta photo)
    fontSize = 34,
    bg = "#12a38a",
    hover = "#0f8e78",
    fontFamily = "monospace",
  ...
  const container = this.add.container(x, y, [g, txt]).setDepth(depth);
  ...
  container.on("pointerout", () => draw(currentBg));
  ...
  container.setText = (newLabel) => {
    ...
    container.setSize(w, h);
    draw(currentBg);
    if (container.input && container.input.hitArea && container.input.hitArea.setTo) {
      container.input.hitArea.setTo(-w / 2, -h / 2, w, h);
    }
    return container;
  };
...
showShop(){
  ...
  if (skinState && Array.isArray(skinState.skins)) {
    skinState.skins.forEach((skin, i) => {
      ...
      const iconScale = iconSize / Math.max(icon.width || iconSize, icon.height || iconSize);
  ...
  const close = this.add.text(panelX, panelTop + panelHeight*0.92, t("COMMON_CLOSE"), {
    fontFamily: "monospace",
    fontSize: 40,
    color: "#fff",
    backgroundColor: "#0db187",
  ...
  showIntroSlides(){
    ...
    skipTxt.on("pointerdown", destroyAll);
    overlay.on("pointerdown", destroyAll);
    ...
    prevTxt.on("pointerdown", () => goStep(step - 1));
    nextTxt.on("pointerdown", () => goStep(step + 1));
    startBtn.on("pointerdown", destroyAll);
...
class GameScene extends Phaser.Scene {
  ...
  update(){
    ...
    this.goldPigs.children.iterate(pig => {
      if (!pig || !pig.active) return;

      if (pig.x < -KILL_MARGIN) {
        pig.destroy();
        return;
      }

      // Même pickup "aimant" que les Borgy Coins
      if (this.player && this.player.active) {
        const dx = pig.x - this.player.x;
        const dy = pig.y - this.player.y;
        const distSq = dx * dx + dy * dy;
  ...
  spawnPair(silentFirst){
    ...
    // COCHON GOLD (mode GOLD uniquement) : 1 apparition tous les 20 tuyaux
    if (this.isGoldPipes && this.started && (this.pairsSpawned % 20 === 0)) {
      this.spawnGoldPig(x, gapCenterY, this.curSpeed);
    }
...
spawnBorgyCoin(x, y, vx){
  ...
  // ====== gestion du contact Borgy / cochon GOLD ======
  ...
  // ====== Spawn du cochon GOLD (même taille visuelle que les Borgy Coins) ======
  spawnGoldPig(x, y, vx){
    if (!this.textures.exists("gold_pig")) return;
    ...
    // Même hitbox que la pièce (basée sur le bonus SwissBorg)
    let targetSide = 140;
    try {
      const bonusTex = this.textures.get("bonus_sb");
      const bonusImg = bonusTex.getSourceImage?.();
  ...
  onCollectGoldPig(x, y){
    // Récompense simple (mode GOLD uniquement)
    const gain = 10;
    ...
    if (!this.game._muted && this.sfxCoin){
      try { this.sfxCoin.play(); } catch(e){}
    }

    // Petit pop text
    try {
      const pop = this.add.text(x, y - 22, `+${gain}`, {
        fontFamily:"monospace",
        fontSize: 28,
        color: "#ffe08a",
        stroke: "#000000",
      ...
      this.tweens.add({
        ...
        duration: 650,
        ease: "Sine.out",
        onComplete: () => { try { pop.destroy(); } catch(e){} }
      });
    } catch(e){}
  }
...
_finalGameOver(){
  ...
  if (this.goldPigs) this.goldPigs.clear(true, true);
