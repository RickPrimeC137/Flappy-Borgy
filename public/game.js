...
makeBtn(x, y, label, cb, opts = {}) {
  ...
  showLeaderboard(isHard = false) {
    ...
    // Même "skin" graphique que le tuto
    ...
    const panelHeight = isMobileLike ? H * 0.68 : H * 0.62;
    ...
    // Close "✕" en haut à droite (comme le tuto)
    const closeX = W/2 + panelWidth*0.45;
    const closeY = H/2 - panelHeight*0.43;
    const closeTop = this.add.text(closeX, closeY, "✕", {
      fontFamily: "monospace",
      fontSize: 26,
      color: "#c7fff6"
    }).setOrigin(1, 0.5).setDepth(depthPanel + 2).setInteractive({ useHandCursor: true });
    elements.push(closeTop);

    // Header
    const iconTxt = this.add.text(W/2, H/2 - panelHeight*0.36, "📜", {
      fontFamily: "monospace",
    ...
    const titleTxt = this.add.text(W/2, H/2 - panelHeight*0.20, t("QUESTS_TITLE"), {
      fontFamily: "monospace",
      fontSize: isMobileLike ? 42 : 46,
      color: "#17d3c7",
      align: "center",
    ...
    if (isHard){
      const hardHint = this.add.text(W/2, H/2 - panelHeight*0.14, t("QUESTS_HARD_HINT"), {
        fontFamily: "monospace",
    ...
    // Contenu
    const contentW = panelWidth * 0.84;
    const leftX    = W/2 - contentW/2;
    const rightX   = W/2 + contentW/2;

    const rowStartY = H/2 - panelHeight*0.03;
    const rowGap    = isMobileLike ? 118 : 124;

    const barH = isMobileLike ? 12 : 12;
    const barR = 6;

    const makeBar = (x, y, w, h, pct, done) => {
      const bg = this.add.graphics().setDepth(depthPanel + 1);
      bg.fillStyle(0x334155, 0.9);
      bg.fillRoundedRect(x, y, w, h, barR);
      elements.push(bg);

      const fw = Math.max(0, Math.min(w, w * pct));
      const r  = Math.max(2, Math.min(barR, fw/2));
      const fg = this.add.graphics().setDepth(depthPanel + 2);
      fg.fillStyle(done ? 0x0db187 : 0x17d3c7, 1);
      if (fw > 0.5) fg.fillRoundedRect(x, y, fw, h, r);
      elements.push(fg);
    };
    ...
    data.quests.forEach((q, i) => {
      ...
      const pct = Math.min(1, (q.target > 0 ? (q.progress / q.target) : 0));
      const progressTxt = `${Math.min(q.progress, q.target)}/${q.target}`;

      const prefix = q.done ? "✅ " : "• ";
      const title = this.add.text(leftX, y, prefix + q.title, {
        fontFamily: "monospace",
        fontSize: isMobileLike ? 26 : 30,
        color: q.done ? "#b3ffcf" : "#ffffff",
        wordWrap: { width: contentW * 0.72 }
      }).setOrigin(0, 0.5).setDepth(depthPanel + 2);
      elements.push(title);

      const prog = this.add.text(rightX, y, progressTxt, {
        fontFamily: "monospace",
      ...
      const barY = y + (isMobileLike ? 28 : 30);
      makeBar(leftX, barY, contentW, barH, pct, q.done);

      const rewardTxt = isHard ? `${q.reward} (x2)` : q.reward;
      const reward = this.add.text(leftX, barY + barH + 18, `${t("QUESTS_REWARD_LABEL")} ${rewardTxt}`, {
        fontFamily: "monospace",
        fontSize: isMobileLike ? 18 : 18,
        color: "#c3ede5"
      }).setOrigin(0, 0.5).setDepth(depthPanel + 2);
    ...
    // Footer
    const totalTxt = this.add.text(W/2, H/2 + panelHeight*0.33, `${t("QUESTS_TOTAL_COINS")} ${totalAfter} 🪙`, {
      fontFamily: "monospace",
      fontSize: isMobileLike ? 24 : 26,
      color: "#cffff1"
    }).setOrigin(0.5).setDepth(depthPanel + 2);
    elements.push(totalTxt);

    const closeBtn = this.add.text(W/2, H/2 + panelHeight*0.43, t("COMMON_CLOSE"), {
      fontFamily: "monospace",
      fontSize: isMobileLike ? 28 : 30,
      color: "#ffffff",
      backgroundColor: "#0db187",
    ...
    closeTop.on("pointerdown", destroyAll);
