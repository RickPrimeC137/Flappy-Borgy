...
const GOLD_PIG_EVERY = 20; // cochon en mode Gold (tous les 20 tuyaux)
...
class PreloadScene extends Phaser.Scene {
  ...
  preload(){
    ...
    this.load.image("gold_pig",  "gold_pig.png"); // <-- ajoute ton PNG dans /assets
...
class GameScene extends Phaser.Scene {
  ...
  spawnPair(silentFirst){
    ...
    // COCHON (mode Gold uniquement) : 1 apparition toutes les 20 paires de tuyaux
    if (this.started && this.isGoldPipes && (this.pairsSpawned % GOLD_PIG_EVERY === 0)){
      const pigX = x;
  ...
  handleGoldPigOverlap(player, pig){
    ...
    this.onCollectGoldPig(cx, cy);
  }

  // ====== Spawn du cochon (même taille/hitbox que les Borgy coins) ======
  spawnGoldPig(x, y, vx){
    const pig = this.physics.add.image(x, y, "gold_pig")
      .setDepth(8)
      .setScale(0.09) // même scale que spawnBorgyCoin()
      .setImmovable(true);

    pig.body.setAllowGravity(false);
    pig.body.setVelocityX(vx);

    // hitbox identique à celle des Borgy coins (basée sur le bonus SwissBorg)
    let targetSide = 140;
    try {
      const bonusTex = this.textures.get("bonus_sb");
      const bonusImg = bonusTex.getSourceImage?.();
      ...
      console.warn("calc gold pig hitbox error", e);
    }
    ...
    // petite animation "respire" comme la pièce
    this.tweens.add({
      targets: pig,
      scaleX: 0.10 * 0.9,
      duration: 600,
  ...
  onCollectGoldPig(x, y){
    // Récompense spéciale (modifiable) : +20 BorgyCoins (logique car 1 apparition / 20 tuyaux)
    const gain = 20;
    ...
    const floatTxt = this.add.text(x, y, `🐷 +${gain}`, {
      fontFamily:"monospace", fontSize:32, color:"#ffe4ff", stroke:"#000000", strokeThickness:4
