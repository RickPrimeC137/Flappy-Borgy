...
const panel = this.add.rectangle(panelX, panelY, panelW, panelH, 0x05252f, 0.96)
  .setDepth(depth+1);
    ...
    // Shop: icônes plus grandes (lisibles) + ajustement léger du layout
    const startY = H * 0.40;
    const lineH  = 70;

    // Colonnes (centrées dans le panneau)
    const iconSize = Math.round(Math.max(64, Math.min(92, W * 0.075))); // ~76px sur 1024px de large
    const colIcon  = panelLeft + panelW * 0.13;
    const colName  = panelLeft + panelW * 0.24;
    const colPrice = panelLeft + panelW * 0.72;
    const colBtn   = panelLeft + panelW * 0.76;
