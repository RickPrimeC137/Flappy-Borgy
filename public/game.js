...
function ensureBgm(scene, opts = {}) {
  ...
  if (scene.sound.locked) {
    // En webview/mobile, le 1er tap sert souvent à déverrouiller l’audio.
    // On écoute l’event UNLOCKED plutôt que d’accrocher un pointerdown global,
    // ce qui évite de "perdre" le 1er clic sur un bouton.
    scene.sound.once(Phaser.Sound.Events.UNLOCKED, start);
    scene.input.keyboard?.once("keydown-SPACE", () => { if (!scene.sound.locked) start(); });
  } else start();
...
class MenuScene extends Phaser.Scene {
  ...
  create(){
    ...
    const applyMenuBg = () => {
      ...
      // IMPORTANT: quand on revient au menu après un scene.start(), l'ancienne instance
      // de _menuBg peut être "destroyed" => setTexture() ferait planter (this.scene undefined)
      const bgValid = (bg) => !!(bg && bg.scene && bg.active !== false && bg.destroyed !== true);

      if (!bgValid(this._menuBg)) {
        try { this._menuBg?.destroy(); } catch {}
        this._menuBg = this.add.image(W/2, H/2, keyUse).setDepth(-20);
        this._menuBg.setScrollFactor(0);
      } else if (this._menuBg.texture?.key !== keyUse) {
    ...
    // Nettoyage quand on quitte le menu (évite les refs mortes au retour)
...
// LEADERBOARD paginé dans le menu (style "tuto")
showLeaderboard(isHard = false) {
  ...
  const panelW = isMobileLike ? W * 0.90 : W * 0.82;
  const panelH = isMobileLike ? H * 0.72 : H * 0.66;
  const panelX = W / 2;
  const panelY = H * 0.52;
  ...
  const panel = this.add.rectangle(panelX, panelY, panelW, panelH, 0x08313a, 0.96)
    .setDepth(depth + 1);
  elements.push(panel);

  const border = this.add.graphics().setDepth(depth + 2);
  border.lineStyle(6, 0x17d3c7, 1);
  border.strokeRoundedRect(panelX - panelW/2, panelY - panelH/2, panelW, panelH, 18);
  elements.push(border);

  const titleText = isHard ? t("LEADERBOARD_TITLE_HARD") : t("LEADERBOARD_TITLE");
  const title = this.add.text(panelX, panelY - panelH * 0.40, titleText, {
    fontFamily: "Georgia,serif",
    fontSize: isMobileLike ? 52 : 60,
    color: "#ffffff"
  }).setOrigin(0.5).setDepth(depth + 3);
  elements.push(title);

  const pageLabel = this.add.text(panelX, panelY - panelH * 0.32, `${t("LEADERBOARD_PAGE_LABEL")} 1`, {
    fontFamily: "monospace",
    fontSize: isMobileLike ? 20 : 22,
    color: "#c7fff6"
  }).setOrigin(0.5).setDepth(depth + 3);
  elements.push(pageLabel);

  // Onglets (Global / Semaine / Mois)
  const scopeY = panelY - panelH * 0.24;
  const tabFont = isMobileLike ? 20 : 22;

  const mkTab = (x, label) => {
    const btn = this.add.text(x, scopeY, label, {
      fontFamily: "monospace",
      fontSize: tabFont,
      color: "#ffffff",
    ...
    btn.on("pointerover", () => {
      if (btn._active !== true) btn.setBackgroundColor("#0e8595");
    });
    btn.on("pointerout", () => {
      if (btn._active !== true) btn.setBackgroundColor("#0b7285");
    });
  ...
  const btnGlobal = mkTab(panelX - panelW * 0.22, t("LEADERBOARD_SCOPE_GLOBAL"));
  const btnWeek   = mkTab(panelX,               t("LEADERBOARD_SCOPE_WEEK"));
  const btnMonth  = mkTab(panelX + panelW * 0.22, t("LEADERBOARD_SCOPE_MONTH"));

  const refreshScopeButtons = () => {
    const set = (b, active) => {
      b._active = !!active;
      b.setBackgroundColor(active ? "#0db187" : "#0b7285");
    };
    set(btnGlobal, currentScope === "all");
    set(btnWeek,   currentScope === "week");
    set(btnMonth,  currentScope === "month");
  };
  refreshScopeButtons();

  // Colonnes
  const colRankX  = panelX - panelW * 0.42;
  const colNameX  = panelX - panelW * 0.34;
  const colScoreX = panelX + panelW * 0.42;

  const headerY = panelY - panelH * 0.14;
  const header = this.add.text(colNameX, headerY, "Player", {
    fontFamily: "monospace",
    fontSize: isMobileLike ? 18 : 20,
    color: "#9be7ff"
  }).setOrigin(0, 0.5).setDepth(depth + 3);
  const headerScore = this.add.text(colScoreX, headerY, "Score", {
    fontFamily: "monospace",
    fontSize: isMobileLike ? 18 : 20,
    color: "#9be7ff"
  }).setOrigin(1, 0.5).setDepth(depth + 3);
  const headerRank = this.add.text(colRankX, headerY, "#", {
    fontFamily: "monospace",
    fontSize: isMobileLike ? 18 : 20,
    color: "#9be7ff"
  }).setOrigin(0, 0.5).setDepth(depth + 3);

  elements.push(header, headerScore, headerRank);

  const startY = panelY - panelH * 0.06;
  const lineH  = isMobileLike ? 52 : 56;

  const closeY = panelY + panelH * 0.40;

  const makeActionBtn = (x, label) => {
    const btn = this.add.text(x, closeY, label, {
      fontFamily: "monospace",
      fontSize: isMobileLike ? 32 : 40,
      color: "#ffffff",
      backgroundColor: "#0db187",
      padding: { left: 22, right: 22, top: 10, bottom: 10 }
    }).setOrigin(0.5).setDepth(depth + 3).setInteractive({ useHandCursor: true });

    btn.on("pointerover", () => btn.setBackgroundColor("#11c69a"));
    btn.on("pointerout",  () => btn.setBackgroundColor("#0db187"));

    elements.push(btn);
    return btn;
  };

  const close = makeActionBtn(panelX, t("COMMON_CLOSE"));

  const makeArrowBtn = (x, label) => {
    const btn = this.add.text(x, closeY, label, {
      fontFamily: "monospace",
      fontSize: isMobileLike ? 28 : 36,
      color: "#ffffff",
      backgroundColor: "#0b7285",
      padding: { left: 18, right: 18, top: 10, bottom: 10 }
    }).setOrigin(0.5).setDepth(depth + 3).setInteractive({ useHandCursor: true });
  ...
  const prevBtn = makeArrowBtn(panelX - panelW * 0.28, "◀");
  const nextBtn = makeArrowBtn(panelX + panelW * 0.28, "▶");
  ...
  const refreshButtonsState = (listLength) => {
    ...
    setEnabled(prevBtn, currentPage > 1);
    setEnabled(nextBtn, listLength >= 10);
  };
  ...
  const loadPage = async (page) => {
    ...
    list.slice(0, 10).forEach((row, i) => {
      ...
      const bg = this.add.rectangle(panelX, y, panelW * 0.90, lineH * 0.78, 0x000000, (i % 2 === 0) ? 0.16 : 0.10)
        .setDepth(depth + 2);
      rows.push(bg);

      const rankTxt = this.add.text(colRankX, y, String((page - 1) * 10 + i + 1).padStart(2, "0") + ".", {
        fontFamily: "monospace",
        fontSize: isMobileLike ? 30 : 34,
        color: "#c7fff6"
      }).setDepth(depth + 3).setOrigin(0, 0.5);

      const nameTxt = this.add.text(colNameX, y, row.name || "Player", {
        fontFamily: "monospace",
        fontSize: isMobileLike ? 30 : 34,
        color: "#ffffff"
      }).setDepth(depth + 3).setOrigin(0, 0.5);

      const scoreTxt = this.add.text(colScoreX, y, String(row.best), {
        fontFamily: "monospace",
        fontSize: isMobileLike ? 30 : 34,
        color: "#cffff1"
...
const panel = this.add.rectangle(panelX, panelY, panelW, panelH, 0x05252f, 0.96)
  .setDepth(depth+1);
    elements.push(panel);
...
// Colonnes (centrées dans le panneau)
const iconSize = 44;
const colIcon  = panelLeft + panelW * 0.10;
const colName  = panelLeft + panelW * 0.16;
const colPrice = panelLeft + panelW * 0.72;
const colBtn   = panelLeft + panelW * 0.75;
    ...
    if (skinState && Array.isArray(skinState.skins)) {
      skinState.skins.forEach((skin, i) => {
        ...
        // Miniature du skin (sprite déjà chargé)
...
class GameScene extends Phaser.Scene {
  ...
  _finalGameOver(){
    ...
    menuBtn.on("pointerdown", () => {
      ...
      // Transition robuste (évite le crash "reading 'sys'" si la scene n'existe pas dans le build chargé)
      try {
        this.scene.start("menu");
      } catch (e) {
        console.warn("scene.start('menu') a échoué, fallback vers preload/reload", e);
        try {
          this.scene.start("preload"); // preload -> menu
        } catch (e2) {
...
// LEADERBOARD paginé dans la GameScene (après Game Over) — style "tuto"
showLeaderboard(isHard = false){
  const W = this.scale.width;
  const H = this.scale.height;
  const depth = 300;
