...
class PreloadScene extends Phaser.Scene {
  ...
  preload(){
    ...
    // Cochon volant (uniquement utilisé en mode Gold)
...
class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    // Cochon volant (mode Gold uniquement)
    this.physics.add.overlap(
      this.player,
      this.goldPigs,
...
// COCHON VOLANT (mode Gold uniquement) : 1 apparition tous les 20 tuyaux (= 10 paires)
if (this.isGoldPipes && this.started && this.pairsSpawned > 0 && (this.pairsSpawned % 10 === 0)) {
  const pigX = x;
...
// ====== gestion du contact Borgy / cochon volant (mode Gold) ======
handleGoldPigOverlap(player, pig){
  if (!pig || !pig.active) return;
  const px = pig.x;
  const py = pig.y;
  pig.disableBody(true, true);
  // Même récompense qu'une Borgy Coin (on réutilise la logique existante)
  this.onCollectBorgyCoin(px, py);
}

// ====== Spawn du cochon volant (taille identique aux Borgy Coins) ======
spawnGoldPig(x, y, vx){
  // si la texture n'est pas chargée, on n'affiche rien (évite un crash silencieux)
  ...
  // Hitbox identique aux Borgy Coins (calée sur bonus_sb)
  let targetSide = 140;
  try {
    const bonusTex = this.textures.get("bonus_sb");
    const bonusImg = bonusTex.getSourceImage?.();
