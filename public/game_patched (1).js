...
// --- icône du skin (à gauche du nom) ---
const iconX = W * 0.12;
const nameX = W * 0.19;
const desiredIconH = 44; // hauteur visuelle ~ en px
let iconImg = null;
try {
  if (this.textures.exists(skin.key)) {
    const tex = this.textures.get(skin.key);
    const srcImg = tex?.getSourceImage?.();
    let visH = srcImg?.height || 1;
    if (srcImg) {
      const b = getVisibleBounds(srcImg);
      if (b && b.h) visH = b.h;
    }
    const scale = desiredIconH / Math.max(1, visH);
    iconImg = this.add.image(iconX, y, skin.key)
      .setOrigin(0.5, 0.5)
      .setScale(scale)
      .setDepth(depth+2);
    elements.push(iconImg);
        ...
        const nameTxt = this.add.text(nameX, y, skin.name, {
          fontFamily: "monospace",
          fontSize: 26,
          color: "#ffffff"
